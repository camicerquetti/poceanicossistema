...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query_compra = "INSERT INTO comprax (emision, proveedor, categoria, subtotal, iva, total, estado, metodo_pago, descripcion, vendedor, cuenta, vencimiento)
    ...
    if ($conn->query($query_compra) === TRUE) {
        ...
        foreach ($_POST['productos'] as $index => $producto_id) {
            ...
            if (is_numeric($producto_id)) {
                ...
                if ($producto_resultado && $producto_resultado->num_rows > 0) {
                    ...
                    $query_producto_compras = "INSERT INTO productos_comprax (compra_id, producto_id, cantidad, precio_unitario, peso_kg)
        ...
        $query_actualizar_compra = "UPDATE comprax SET subtotal = $subtotal, iva = $iva, total = $total, estado = '$estado' WHERE id = $compra_id";
