...
$compra = $conn->query("SELECT * FROM compras WHERE id = $compra_id")->fetch_assoc();
// var_dump($compra);

$productosCompra = $conn->query("SELECT * FROM productos_compra WHERE compra_id = $compra_id")->fetch_all(MYSQLI_ASSOC);
    ...
    if ($conn->query($query_actualizar) === TRUE) {
        ...
        foreach ($_POST['productos'] as $index => $producto_id) {
            ...
            if (is_numeric($producto_id)) {
                $query_producto = "INSERT INTO productos_compra (compra_id, producto_id, cantidad, precio_unitario, peso_kg) 
