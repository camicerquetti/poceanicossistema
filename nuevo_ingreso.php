...
if ($result->num_rows > 0) {
    ...
    // Extraer el número de factura (la parte después del último '-')
...
// Verificar el número de factura generado
echo "Número de factura generado: " . $factura_afip;
    ...
    // Insertar ingreso en la base de datos, incluyendo el número de factura
    $sql = "INSERT INTO ingresos (fecha, vencimiento, tipo_ingreso, descripcion, monto, estado, empleado_responsable, metodo_pago, metodo_transporte, proveedor, tipo_factura, subtotal, iva, total, cliente, id_cuenta, vendedor_nombre, factura_afip) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
        ...
        if ($stmt->execute()) {
            ...
            foreach ($_POST['productos'] as $index => $producto_id) {
                ...
                if (isset($productos_data[$producto_id]) && $cantidad > 0 && $precio > 0) {
                    ...
                    $sql_detalle = "INSERT INTO ingreso_productos (ingreso_id, producto_id, cantidad, precio, iva,peso_kg) VALUES (?, ?, ?, ?, ?,?)";
